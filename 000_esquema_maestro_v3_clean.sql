...
ALTER TABLE notificaciones          ADD COLUMN IF NOT EXISTS actualizado_en TIMESTAMPTZ DEFAULT NOW();
...
-- Must be before indices that reference them
-- ============================================================
...
-- ============================================================
-- VISTAS - Drop first so ALTER COLUMN TYPE below doesn't conflict
-- ============================================================
DROP VIEW IF EXISTS contenido_cms_activo CASCADE;
DROP VIEW IF EXISTS motorizados_disponibles CASCADE;
DROP VIEW IF EXISTS resumen_ganancias_motorizados CASCADE;

-- ============================================================
-- ALTER COLLATION EN COLUMNAS
-- (Must happen before views reference these columns)
-- ============================================================
...
SELECT
  m.id, m.perfil_id, m.tipo_vehiculo, m.placa,
  m.ubicacion_actual, m.disponible, m.ordenes_activas,
  m.sucursal_id, m.activo, m.creado_en, m.actualizado_en,
  p.nombre_completo,
  p.telefono
FROM   motorizados m
...
-- ============================================================
-- FUNCIONES - Drop first to avoid PG15 param name conflicts
-- ============================================================
DROP FUNCTION IF EXISTS normalizar_texto CASCADE;
DROP FUNCTION IF EXISTS buscar_motorizados_disponibles CASCADE;
DROP FUNCTION IF EXISTS calcular_precio_servicio CASCADE;
DROP FUNCTION IF EXISTS actualizar_timestamp CASCADE;
DROP FUNCTION IF EXISTS actualizar_ordenes_activas_motorizado CASCADE;
DROP FUNCTION IF EXISTS sincronizar_estado_solicitud CASCADE;
DROP FUNCTION IF EXISTS actualizar_estado_contenido_cms CASCADE;
DROP FUNCTION IF EXISTS crear_perfil_nuevo_usuario CASCADE;
DROP FUNCTION IF EXISTS actualizar_vector_busqueda_negocio CASCADE;
DROP FUNCTION IF EXISTS actualizar_vector_busqueda_producto CASCADE;
DROP FUNCTION IF EXISTS buscar_negocios CASCADE;
DROP FUNCTION IF EXISTS buscar_productos CASCADE;
DROP FUNCTION IF EXISTS obtener_mi_rol CASCADE;
DROP FUNCTION IF EXISTS es_administrador CASCADE;
...
RETURNS BOOLEAN LANGUAGE SQL STABLE AS $$
  SELECT obtener_mi_rol() IN ('admin', 'super_admin', 'administrador', 'super_administrador');
$$;
...
CREATE POLICY "cms_config_admin_write" ON public.cms_config FOR ALL USING (
  EXISTS (SELECT 1 FROM public.perfiles WHERE id = auth.uid() AND rol IN ('super_admin', 'admin', 'super_administrador', 'administrador') AND activo = true)
);
...
-- Catalogo de 7 servicios (solo si no existen ya)
DO $$ DECLARE v_sid UUID; BEGIN
  SELECT id INTO v_sid FROM sucursales ORDER BY creado_en LIMIT 1;
  IF NOT EXISTS (SELECT 1 FROM catalogo_servicios WHERE nombre = 'Mandado' AND nombre_mostrar IS NOT NULL) THEN
    INSERT INTO catalogo_servicios (sucursal_id, nombre, tipo_servicio, nombre_mostrar, descripcion, precio_base, precio_por_parada, precio_manual, tipo_precio, orden)
    VALUES (v_sid, 'Mandado', 'mandado', 'Mandado', 'Mandados dentro de Ocotal. C por parada.', 40.00, 40.00, FALSE, 'fijo'::tipo_precio, 1);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM catalogo_servicios WHERE nombre = 'Delivery' AND nombre_mostrar IS NOT NULL) THEN
    INSERT INTO catalogo_servicios (sucursal_id, nombre, tipo_servicio, nombre_mostrar, descripcion, precio_base, precio_por_parada, precio_manual, tipo_precio, orden)
    VALUES (v_sid, 'Delivery', 'delivery', 'Delivery', 'Entrega a domicilio. C por parada.', 40.00, 40.00, FALSE, 'fijo'::tipo_precio, 2);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM catalogo_servicios WHERE nombre = 'Encomienda' AND nombre_mostrar IS NOT NULL) THEN
    INSERT INTO catalogo_servicios (sucursal_id, nombre, tipo_servicio, nombre_mostrar, descripcion, precio_base, precio_por_parada, precio_manual, tipo_precio, orden)
    VALUES (v_sid, 'Encomienda', 'encomienda', 'Encomienda', 'Envio de paquetes. C por parada.', 40.00, 40.00, FALSE, 'fijo'::tipo_precio, 3);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM catalogo_servicios WHERE nombre = 'Viaje Expreso' AND nombre_mostrar IS NOT NULL) THEN
    INSERT INTO catalogo_servicios (sucursal_id, nombre, tipo_servicio, nombre_mostrar, descripcion, precio_base, precio_por_parada, precio_manual, tipo_precio, orden)
    VALUES (v_sid, 'Viaje Expreso', 'viaje_expreso', 'Viaje Expreso', 'Viajes al norte, centro y pacifico.', 0.00, 0.00, TRUE, 'manual'::tipo_precio, 4);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM catalogo_servicios WHERE nombre = 'Transporte' AND nombre_mostrar IS NOT NULL) THEN
    INSERT INTO catalogo_servicios (sucursal_id, nombre, tipo_servicio, nombre_mostrar, descripcion, precio_base, precio_por_parada, precio_manual, tipo_precio, orden)
    VALUES (v_sid, 'Transporte', 'transporte', 'Transporte', 'Transporte de personas o carga.', 0.00, 0.00, TRUE, 'manual'::tipo_precio, 5);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM catalogo_servicios WHERE nombre = 'Acarreo' AND nombre_mostrar IS NOT NULL) THEN
    INSERT INTO catalogo_servicios (sucursal_id, nombre, tipo_servicio, nombre_mostrar, descripcion, precio_base, precio_por_parada, precio_manual, tipo_precio, orden)
    VALUES (v_sid, 'Acarreo', 'acarreo', 'Acarreo', 'Traslado de materiales o muebles.', 0.00, 0.00, TRUE, 'manual'::tipo_precio, 6);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM catalogo_servicios WHERE nombre = 'Mudanza' AND nombre_mostrar IS NOT NULL) THEN
    INSERT INTO catalogo_servicios (sucursal_id, nombre, tipo_servicio, nombre_mostrar, descripcion, precio_base, precio_por_parada, precio_manual, tipo_precio, orden)
    VALUES (v_sid, 'Mudanza', 'mudanza', 'Mudanza', 'Mudanza completa.', 0.00, 0.00, TRUE, 'manual'::tipo_precio, 7);
  END IF;
END $$;
...
EXCEPTION WHEN undefined_table THEN RAISE NOTICE 'Publication supabase_realtime no existe, omitiendo';
  WHEN duplicate_object THEN RAISE NOTICE 'solicitudes_servicio ya en publication, omitiendo';
END $$;

DO $$ BEGIN
  ALTER PUBLICATION supabase_realtime ADD TABLE asignaciones;
EXCEPTION WHEN undefined_table THEN RAISE NOTICE 'Publication supabase_realtime no existe, omitiendo';
  WHEN duplicate_object THEN RAISE NOTICE 'asignaciones ya en publication, omitiendo';
END $$;

DO $$ BEGIN
  ALTER PUBLICATION supabase_realtime ADD TABLE motorizados;
EXCEPTION WHEN undefined_table THEN RAISE NOTICE 'Publication supabase_realtime no existe, omitiendo';
  WHEN duplicate_object THEN RAISE NOTICE 'motorizados ya en publication, omitiendo';
END $$;

DO $$ BEGIN
  ALTER PUBLICATION supabase_realtime ADD TABLE notificaciones;
EXCEPTION WHEN undefined_table THEN RAISE NOTICE 'Publication supabase_realtime no existe, omitiendo';
  WHEN duplicate_object THEN RAISE NOTICE 'notificaciones ya en publication, omitiendo';
END $$;

DO $$ BEGIN
  ALTER PUBLICATION supabase_realtime ADD TABLE sesiones_chat_ia;
EXCEPTION WHEN undefined_table THEN RAISE NOTICE 'Publication supabase_realtime no existe, omitiendo';
  WHEN duplicate_object THEN RAISE NOTICE 'sesiones_chat_ia ya en publication, omitiendo';
END $$;
